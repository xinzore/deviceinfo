...
const getSectionValueByKey = (phone: any, fieldKey: string) => {
  const sections = phone?.specs?.sections;
  if (!sections || typeof sections !== 'object') return undefined;
  for (const section of Object.values(sections)) {
    if (!section || typeof section !== 'object') continue;
    if (fieldKey in (section as Record<string, unknown>)) {
  ...
  try {
    ...
    const summary = phones
      ...
      .map((phone: any) => ({
        ...
        table: {
          ekranBoyutu: getSectionValueByKey(phone, 'ekranBoyutu'),
          dahiliDepolama: getSectionValueByKey(phone, 'dahiliDepolama'),
          bellekRam: getSectionValueByKey(phone, 'bellekRam'),
          bataryaKapasitesiTipik: getSectionValueByKey(phone, 'bataryaKapasitesiTipik'),
        },
